...
cells.append(md(
    ...
    "",
    "To avoid mutating the production workspace manifest, notebook cargo commands run",
    "through a temporary standalone cargo root under `target-local/colab-runtime`.",
))
...
cells.append(code(
    ...
    "",
    "RUNTIME_DIR  = CRATE_ROOT / 'target-local' / 'colab-runtime'",
    "MANIFEST_SRC = CRATE_ROOT / 'Cargo.toml'",
    "MANIFEST_DST = RUNTIME_DIR / 'Cargo.toml'",
    ...
    "",
    "def _prepare_runtime_crate():",
    "    RUNTIME_DIR.mkdir(parents=True, exist_ok=True)",
    "    manifest_text = MANIFEST_SRC.read_text()",
    "    if '[workspace]' not in manifest_text:",
    "        manifest_text = manifest_text.rstrip() + '\\n\\n[workspace]\\nresolver = \"2\"\\n'",
    "    MANIFEST_DST.write_text(manifest_text)",
    "    for name in ['src', 'examples', 'tests', 'data', 'figures', 'scripts', 'Cargo.lock', 'README.md']:",
    "        src = CRATE_ROOT / name",
    "        if src.exists():",
    "            _link_or_copy(src, RUNTIME_DIR / name)",
    "    return RUNTIME_DIR",
    "",
    "REPO = _prepare_runtime_crate()",
    "",
    "print(f'Crate root : {CRATE_ROOT}')",
    "print(f'Cargo root : {REPO}')",
