...
cells.append(md(
    ...
    "",
    "> **ZERO OVERCLAIMING.**  DSFB is a read-only observer.  It produces grammar",
    "> annotations on residuals; it does **not** predict failures, detect anomalies",
    "> in any ML sense, control systems, or issue alarms.  Fault labels are used",
    "> *solely* as post-hoc annotation metadata — they were never provided to the",
    "> DSFB engine during processing.",
    "",
    "---",
    "",
...
cells.append(md(
    ...
    "",
    "**Colab:** mount Google Drive and set `DRIVE_REPO_PATH` to your copy of the repo.",
))

# ── Cell 06 ── Paths ──────────────────────────────────────────────────────────
cells.append(code(
    "DRIVE_REPO_PATH = '/content/drive/MyDrive/dsfb-oil-gas'  # edit: workspace root",
    "",
    "if IN_COLAB:",
    "    try:",
    "        from google.colab import drive",
    "        drive.mount('/content/drive', force_remount=False)",
    "        WORKSPACE_ROOT = pathlib.Path(DRIVE_REPO_PATH)",
    "        if not WORKSPACE_ROOT.exists():",
    "            raise FileNotFoundError(f'Workspace not found at {WORKSPACE_ROOT}')",
    "    except Exception as exc:",
    "        print(f'Drive issue: {exc}')",
    "        WORKSPACE_ROOT = pathlib.Path('/content/dsfb-oil-gas')",
    "    CRATE_ROOT = WORKSPACE_ROOT / 'crates' / 'dsfb-oil-gas'",
    ...
    "    OUTPUT_DIR = (NB_DIR / '..').resolve()",
    "    WORKSPACE_ROOT = (OUTPUT_DIR / '../..').resolve()",
    "    CRATE_ROOT = WORKSPACE_ROOT / 'crates' / 'dsfb-oil-gas'",
    "    if not CRATE_ROOT.exists():",
    "        CRATE_ROOT = OUTPUT_DIR  # old single-crate layout fallback",
    "",
