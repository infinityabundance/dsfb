...
| DEFINITION | DEF-GRAMMAR-EVIDENCE | Grammar evidence summary | `src/engine/semantics/retrieval.rs` | 497 | Reduces grammar trajectory state into counts and regime tags used by semantic retrieval. |
...
| ALGORITHM | ALG-SEMANTIC-INDEX | Deterministic semantic prefilter index | `src/engine/semantics/retrieval.rs` | 376 | Builds reproducible candidate buckets for larger heuristic banks without replacing exact validation. |
...
| CLAIM | CLM-RETRIEVAL-SCALING-REPORT | Retrieval scaling evidence | `src/engine/semantics/retrieval.rs` | 423 | Exports deterministic candidate-count scaling observations for indexed versus fallback retrieval paths. |
