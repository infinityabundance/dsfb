...
| DEFINITION | DEF-GRAMMAR-EVIDENCE | Grammar evidence summary | `src/engine/semantics/retrieval/scoring.rs` | 17 | Reduces grammar trajectory state into counts and regime tags used by semantic retrieval. |
...
| ASSUMPTION | ASM-BOUNDED-ONLINE-HISTORY | Bounded online history window | `src/live/mod.rs` | 113 | The deployment path retains only the last N residual samples in a fixed-capacity ring buffer. |
...
| ALGORITHM | ALG-BOUNDED-ONLINE-STEP | Bounded online layered step | `src/live/mod.rs` | 359 | Replays residual to semantics over the fixed trailing window without unbounded live-state growth. |
...
| ALGORITHM | ALG-SEMANTIC-INDEX | Deterministic semantic prefilter index | `src/engine/semantics/retrieval/index.rs` | 42 | Builds reproducible candidate buckets for larger heuristic banks without replacing exact validation. |
| ALGORITHM | ALG-SEMANTIC-RETRIEVAL | Typed semantic retrieval | `src/engine/semantics/retrieval.rs` | 73 | Applies admissibility, regime, scope, and compatibility filtering to conservative semantic interpretation. |
...
| CLAIM | CLM-RETRIEVAL-SCALING-REPORT | Retrieval scaling evidence | `src/engine/semantics/retrieval/index.rs` | 89 | Exports deterministic candidate-count scaling observations for indexed versus fallback retrieval paths. |
...
| INTERFACE | IFACE-C-ABI-LIFECYCLE | C ABI engine lifecycle | `ffi/src/lib.rs` | 291 | Creates the bounded live engine handle exposed to legacy hosts. |
| INTERFACE | IFACE-C-ABI-STATUS | Stable C ABI status surface | `ffi/src/lib.rs` | 122 | Maps layered live-engine status into a repr-C numeric interface for external callers. |
