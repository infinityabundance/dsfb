...
\newenvironment{remarkbox}{%
  \begin{tcolorbox}[colback=blue!4,colframe=black!30,boxrule=0.4pt,%
    left=4pt,right=4pt,top=3pt,bottom=3pt,parbox=false]%
}{%
  \end{tcolorbox}%
}
...
% =========================================================
\title{
    \LARGE \textbf{DSFB and Computer Graphics: Deterministic Structural Semiotics}\\[0.4em]
    \large A Narrow Wedge for Structural Supervision of Trust-Regulated Temporal Reuse
    }
\author{
  Riaan de Beer\\
  \texttt{predictiverendezvous@proton.me}\\
  Chief Research Advisor\\[0.5em]
  Invariant Forge LLC\\
  \small
  ORCID: \href{https://orcid.org/0009-0006-1155-027X}{0009-0006-1155-027X}\\
  % TODO: insert manuscript DOI after assignment; no manuscript DOI is asserted here because none is verified in the current inputs.
  \href{https://github.com/infinityabundance/dsfb}{https://github.com/infinityabundance/dsfb}\\[0.5em]
  \small Version 1.0
}
...
\begin{table}[H]
\centering
\caption{Per-frame ROI MAE breakdown for Demo A across the five-capture canonical sequence. Values are approximate; see \texttt{generated/canonical\_2026\_q1/} for exact JSON output. Cells marked {[fill]} indicate values available from the crate artifact manifest that were not transcribed here; run \texttt{cargo run -- run-all} to populate.}
...
\midrule
\texttt{fixed\_alpha} & {[fill]} & {[fill]} & {[fill]} & {[fill]} & {[fill]} \\
\texttt{strong\_heuristic} & {[fill]} & {[fill]} & {[fill]} & {[fill]} & {[fill]} \\
\texttt{dsfb\_host\_minimum} & {[fill]} & {[fill]} & {[fill]} & {[fill]} & {[fill]} \\
\texttt{dsfb\_plus\_strong\_heuristic} & {[fill]} & {[fill]} & {[fill]} & {[fill]} & {[fill]} \\
\bottomrule
\end{tabularx}
\end{table}
...
\bibitem{dsfb_main}
R. de Beer,
\emph{DSFB Structural Semiotics Engine for General Systems: A Deterministic Endoduction Framework for Residual-Based Meaning Extraction},
preprint, 2026.
