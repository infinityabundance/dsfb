...
use crate::types::{
    BatteryResidual, DetectionResult, EnvelopeParams, GrammarState, PipelineConfig, Theorem1Result,
};
...
pub fn generate_all_figures(ctx: &FigureContext, output_dir: &Path) -> Result<(), PlotError> {
    ...
    draw_line(
        ...
        &cycles,
        &[(&ctx.capacities.to_vec(), "Measured capacity", &BLUE)],
        Some((eol_capacity, "EOL threshold", &RED)),
    )?;

    draw_line(
        output_dir,
        "fig02_residual_trajectory",
        "Figure 2: Residual Trajectory r_k",
        "Cycle Number",
        "Residual r_k (Ah)",
        &cycles,
        &[(&residuals, "r_k = y_k - mu", &BLUE)],
        Some((0.0, "Zero line", &BLACK)),
    )?;

    draw_line(
        output_dir,
        "fig03_drift_trajectory",
        "Figure 3: Drift Trajectory d_k",
        "Cycle Number",
        "Drift d_k (Ah/cycle)",
        &cycles,
        &[(&drifts, "d_k (windowed drift)", &BLUE)],
        Some((-ctx.config.drift_threshold, "Drift threshold", &RED)),
    )?;

    draw_line(
        output_dir,
        "fig04_slew_trajectory",
        "Figure 4: Slew Trajectory s_k",
        "Cycle Number",
        "Slew s_k",
        &cycles,
        &[(&slews, "s_k (windowed slew)", &BLUE)],
    ...
    draw_detection_comparison(
        output_dir,
        "fig07_detection_comparison",
        &cycles,
        ctx.capacities,
        eol_capacity,
        ctx.dsfb_detection,
        ctx.threshold_detection,
    )?;

    draw_theorem1(
        output_dir,
        "fig08_theorem1_verification",
        &cycles,
        &residuals,
        ctx.envelope,
        ctx.theorem1,
    ...
    draw_scatter(
        output_dir,
        "fig09_semiotic_projection",
        "Figure 9: Semiotic Projection (r_k vs d_k)",
        "Residual r_k (Ah)",
        "Drift d_k (Ah/cycle)",
        &residuals,
        &drifts,
...

    {
        ...
        draw_line(
            output_dir,
            "fig10_cumulative_drift",
            "Figure 10: Cumulative Drift",
            "Cycle Number",
            "Cumulative Drift (Ah)",
            &cycles,
            &[(&cum_drift, "Cumulative drift", &BLUE)],
            Some((0.0, "Zero", &BLACK)),
        )?;
    }
...
fn draw_envelope(
    ...
    chart
        ...
        .map_err(pe)?
        .label("+rho")
        .legend(|(lx, ly)| PathElement::new(vec![(lx, ly), (lx + 20, ly)], RED));
...
fn draw_grammar_timeline(
    ...
    let y: Vec<f64> = trajectory
        .iter()
        .map(|br| match br.grammar_state {
    ...
    root.fill(&WHITE).map_err(pe)?;
    let mut chart = ChartBuilder::on(&root)
        .caption("Figure 6: Grammar State Timeline", ("sans-serif", 16))
        .margin(10)
        .x_label_area_size(35)
        .y_label_area_size(55)
        .build_cartesian_2d(1.0..n as f64, -0.5..2.5)
        .map_err(pe)?;
    chart
        .configure_mesh()
        .x_desc("Cycle Number")
        .y_desc("Grammar State")
        .draw()
        .map_err(pe)?;
...
fn draw_detection_comparison(
    ...
    root.fill(&WHITE).map_err(pe)?;
    let mut chart = ChartBuilder::on(&root)
        .caption("Figure 7: Detection Comparison", ("sans-serif", 16))
        .margin(10)
        .x_label_area_size(35)
        .y_label_area_size(55)
        .build_cartesian_2d(x_min..x_max, y_min..y_max)
    ...
    if let Some(cycle) = threshold.alarm_cycle {
        chart
            ...
            .legend(|(lx, ly)| {
                PathElement::new(vec![(lx, ly), (lx + 20, ly)], RGBColor(128, 0, 128))
            });
    }
    chart
        .configure_series_labels()
...
fn draw_theorem1(
    ...
    root.fill(&WHITE).map_err(pe)?;
    let mut chart = ChartBuilder::on(&root)
        .caption("Figure 8: Theorem 1 Verification", ("sans-serif", 16))
        .margin(10)
        .x_label_area_size(35)
        .y_label_area_size(55)
    ...
    chart
        ...
        .map_err(pe)?
        .label(format!("Thm 1 (t*={})", thm1.t_star))
        .legend(|(lx, ly)| PathElement::new(vec![(lx, ly), (lx + 20, ly)], GREEN));
...
fn draw_scatter(
    ...
    let mut chart = ChartBuilder::on(&root)
        ...
        .x_label_area_size(35)
        .y_label_area_size(55)
        .build_cartesian_2d((x_min - mx)..(x_max + mx), (y_min - my)..(y_max + my))
        .map_err(pe)?;
    chart
        .configure_mesh()
        .x_desc(xlabel)
...
fn draw_lead_time_bars(
    ...
    root.fill(&WHITE).map_err(pe)?;
    let mut chart = ChartBuilder::on(&root)
        .caption("Figure 11: Lead Time Comparison", ("sans-serif", 16))
        .margin(10)
        .x_label_area_size(35)
        .y_label_area_size(55)
    ...
    chart
        .configure_mesh()
        .y_desc("Lead Time (cycles)")
        .draw()
        .map_err(pe)?;

    chart
        .draw_series(std::iter::once(Rectangle::new(
            [(0.5, 0.0), (1.3, dsfb_lead)],
            GREEN.filled(),
        )))
        .map_err(pe)?
        .label(format!("DSFB ({:.0})", dsfb_lead))
        .legend(|(lx, ly)| Rectangle::new([(lx, ly - 5), (lx + 15, ly + 5)], GREEN.filled()));

    chart
        .draw_series(std::iter::once(Rectangle::new(
            [(1.7, 0.0), (2.5, thresh_lead)],
            RGBColor(128, 0, 128).filled(),
        )))
        .map_err(pe)?
        .label(format!("Threshold ({:.0})", thresh_lead))
        .legend(|(lx, ly)| {
            Rectangle::new(
                [(lx, ly - 5), (lx + 15, ly + 5)],
                RGBColor(128, 0, 128).filled(),
...
fn draw_heuristic_bank(dir: &Path, stem: &str) -> Result<(), PlotError> {
...

    {
        let mut chart = ChartBuilder::on(&upper)
            .caption(
                "Figure 12: Heuristics Bank Entry — SEI Growth Motif",
                ("sans-serif", 14),
            )
            .margin(10)
            .x_label_area_size(35)
            .y_label_area_size(55)
            .build_cartesian_2d(0.0..100.0, 1.5..2.1)
            .map_err(pe)?;
        chart
            .configure_mesh()
            .x_desc("Cycle")
