...
//
// Additive helper binary for ablation comparisons.

use clap::Parser;
use dsfb_battery::{resolve_helper_output_dir, run_ablation_workflow, PipelineConfig};
use std::path::PathBuf;

#[derive(Parser, Debug)]
#[command(version, about = "Opt-in ablation comparison helper")]
struct Cli {
    /// Directory containing NASA cell CSV files such as nasa_b0005_capacity.csv
    #[arg(long)]
    data_dir: Option<PathBuf>,

    /// Output directory for additive ablation artifacts
    #[arg(short, long)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let artifact = run_ablation_workflow(&data_dir, &output_dir, &config)?;
    println!(
        "Ablation helper completed for cells: {}",
