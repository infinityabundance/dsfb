    ...
    for (index, summary) in summaries.iter().enumerate() {
        ...
        chart
            .draw_series(std::iter::once(Text::new(
                format!("{}\n{}", summary.cell_id, summary.lead_time_vs_threshold_baseline.unwrap_or(0)),
...
pub fn generate_ablation_comparison_figure(
    ...
    for (index, cell) in cells.iter().enumerate() {
        ...
        for (offset, cycle, color) in [
            (0.10, cell.threshold_baseline.trigger_cycle, RGBColor(128, 0, 128)),
...
pub fn generate_multicell_residual_state_overview(
    ...
    for (area, run) in areas.into_iter().zip(runs.iter()) {
        ...
        let mut chart = ChartBuilder::on(&area)
            .caption(
                format!("{} residual and grammar-state overview", run.summary.cell_id),
                ("sans-serif", 15),
            )
            .margin(10)
