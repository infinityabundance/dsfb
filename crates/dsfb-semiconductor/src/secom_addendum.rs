...
pub fn build_secom_addendum_artifacts(
    ...
    let metric_regrounding =
        build_metric_regrounding(
            dataset,
            residuals,
            baselines,
            metrics,
...
pub fn draw_recurrent_boundary_tradeoff_plot(
    ...
    let root = BitMapBackend::new(path, (TRADEOFF_PLOT_WIDTH, TRADEOFF_PLOT_HEIGHT))
        .into_drawing_area();
    ...
    chart
        ...
        .map_err(plot_error)?;
    chart
        .draw_series(recall_series.into_iter().map(|point| Circle::new(point, 5, BLACK.filled())))
        .map_err(plot_error)?;

    chart
        .draw_series(LineSeries::new(nuisance_series.clone(), &RGBColor(100, 100, 100)))
        .map_err(plot_error)?;
    chart
        .draw_series(
            nuisance_series
...
fn build_recurrent_boundary_tradeoff_curve(
    ...
    suppression_levels
        .into_iter()
        .map(|(level, label)| {
            let mut feature_alerts = vec![vec![false; dataset.labels.len()]; optimized_dsa.traces.len()];
            ...
            for (trace_slot, feature_trace) in optimized_dsa.traces.iter().enumerate() {
                let motif_trace = motif_by_feature
                    ...
                    .unwrap_or_else(|| panic!("missing motif trace {}", feature_trace.feature_index));
                let grammar_trace = grammar_by_feature
                    .get(&feature_trace.feature_index)
                    .unwrap_or_else(|| panic!("missing grammar trace {}", feature_trace.feature_index));
...
fn recurrent_boundary_tradeoff_statement_supported(
    rows: &[RecurrentBoundaryTradeoffRow],
) -> bool {
...
fn build_metric_regrounding(
    ...
    let threshold_signal = (0..dataset.labels.len())
        .map(|run_index| residuals.traces.iter().any(|trace| trace.threshold_alarm[run_index]))
        .collect::<Vec<_>>();
    ...
    let dsfb_precision = optimized_dsa.episode_summary.precursor_quality.unwrap_or_default();
    let numeric_precision =
        episode_precision(&optimized_dsa.run_signals.numeric_primary_run_alert, &failure_mask);

    vec![
        MetricRegroundingRow {
            metric: "investigation_points".into(),
            baseline: "numeric_only_dsa".into(),
        ...
        MetricRegroundingRow {
            metric: "pass_run_nuisance_proxy".into(),
            baseline: "numeric_only_dsa".into(),
            dsfb_value: optimized_dsa.summary.pass_run_nuisance_proxy,
            baseline_value: optimized_dsa.summary.numeric_primary_pass_run_nuisance_proxy,
            delta_percent: relative_reduction(
                optimized_dsa.summary.numeric_primary_pass_run_nuisance_proxy,
                optimized_dsa.summary.pass_run_nuisance_proxy,
            ),
        },
        MetricRegroundingRow {
        ...
        MetricRegroundingRow {
            metric: "mean_lead_time_runs".into(),
            baseline: "numeric_only_dsa".into(),
            dsfb_value: optimized_dsa.summary.mean_lead_time_runs.unwrap_or_default(),
            ...
            delta_percent: relative_gain(
                optimized_dsa
                    ...
                    .mean_lead_time_runs
                    .unwrap_or_default(),
                optimized_dsa.summary.mean_lead_time_runs.unwrap_or_default(),
            ),
        },
        MetricRegroundingRow {
            metric: "mean_lead_time_runs".into(),
        ...
        MetricRegroundingRow {
            ...
            baseline: "ewma".into(),
            dsfb_value: optimized_dsa.summary.mean_lead_time_runs.unwrap_or_default(),
            baseline_value: metrics.lead_time_summary.mean_ewma_lead_runs.unwrap_or_default(),
            delta_percent: relative_gain(
                metrics.lead_time_summary.mean_ewma_lead_runs.unwrap_or_default(),
                optimized_dsa.summary.mean_lead_time_runs.unwrap_or_default(),
            ),
        },
        MetricRegroundingRow {
...
fn build_target_d_regression_analysis(
    ...
    let tradeoff_justification = if strong_suppression_rows.is_empty() {
        ...
        let recall_rows = strong_suppression_rows
            .iter()
            .map(|row| format!("{} => recall {}/{}", row.suppression_label, row.failure_recall, row.failure_runs))
            .collect::<Vec<_>>()
            .join("; ");
        format!(
    ...
    TargetDRegressionAnalysis {
        ...
        why_regression_occurred: format!(
            "The regression is driven by recall-recovery promotions on {} under motif classes {} and grammar states {}. These add pass-only review segments faster than the accepted isolated-pass suppressions can remove them.",
...
fn build_missed_failure_root_cause(failure_driven: &FailureDrivenArtifacts) -> MissedFailureRootCause {
    ...
    MissedFailureRootCause {
        ...
        reason_for_miss: "policy_suppression".into(),
        classification: "recoverable".into(),
        ...
        recovery_feature: case
            ...
            .or_else(|| case.top_contributing_features.first().map(|feature| feature.feature_name.clone())),
        recovery_note: "The former 103/104 limiting case was failure run 2. It remained a Watch-class near-miss under the numeric gate until the bounded S092 structural-support rescue was accepted, after which the selected row reached 104/104.".into(),
    }
}
    ...
    metrics
        ...
        .map(|metric_row| {
            let dsa_row = dsa_by_failure
                ...
                .copied()
                .unwrap_or_else(|| panic!("missing DSA failure row {}", metric_row.failure_run_index));
            let earliest_semantic_match_run = earliest_semantic_match_run(
                semantic_layer,
                metric_row.failure_run_index,
                pre_failure_lookback_runs,
            );
            let earliest_semantic_match_lead_runs =
                earliest_semantic_match_run.map(|run_index| metric_row.failure_run_index - run_index);
            LeadTimeComparisonRow {
                ...
                threshold_minus_dsfb_runs: diff_optional(metric_row.threshold_lead_runs, dsa_row.dsa_lead_runs),
...
fn build_lead_time_explanation(rows: &[LeadTimeComparisonRow]) -> LeadTimeExplanation {
    let mean_dsfb = mean_option(rows.iter().filter_map(|row| row.dsfb_lead_runs.map(|value| value as f64)));
    let mean_threshold =
        mean_option(rows.iter().filter_map(|row| row.threshold_lead_runs.map(|value| value as f64)));
    let mean_semantic = mean_option(
        rows.iter()
            .filter_map(|row| row.earliest_semantic_match_lead_runs.map(|value| value as f64)),
    );
    ...
    LeadTimeExplanation {
        ...
        validation_note: format!(
            "Mean threshold lead is {} runs versus mean DSA lead {} runs; earliest grammar-qualified semantic matches average {} runs. This gap is consistent with DSFB waiting for structured motifs rather than raw deviation alone.",
...
fn build_episode_precision_metrics(
    ...
    let dsfb_pre_failure_episode_count = optimized_dsa.episode_summary.dsa_episodes_preceding_failure;
    let dsfb_precision = optimized_dsa.episode_summary.precursor_quality.unwrap_or_default();
