...
set -euo pipefail
#
# verify_dsfb_pe.sh — structural PE32+ check for the v0.3 dsfb_demo bootable.
#
# Mirrors tools/phosphoric/verify_pe_efi_image.sh but parameterized for
# the v0.3 bootable's structure: 1 COFF symbol (efi_main only), distinct
# trace markers (the DSFB-demo phosphoric: ... lines + first/last
# theorem lines).
#
# Usage: verify_dsfb_pe.sh BOOTX64.EFI

image="${1:?usage: verify_dsfb_pe.sh BOOTX64.EFI}"
...
read_u8() { od -An -j "$1" -N 1 -tu1 "$image" | awk '{print $1}'; }
read_u16le() {
    local b0 b1
    b0="$(read_u8 "$1")"
    b1="$(read_u8 "$(( $1 + 1 ))")"
...
read_hex() { od -An -j "$1" -N "$2" -tx1 -v "$image" | tr -d ' \n'; }
fail() { printf 'invalid v0.3 PE/EFI image %s: %s\n' "$image" "$1" >&2; exit 1; }
...
[ "$symbol_count" -ge 1 ] || fail "expected at least one COFF symbol"
...
printf '%s\n' "== phosphoric: dsfb v0.3 PE/EFI structure verification complete =="
