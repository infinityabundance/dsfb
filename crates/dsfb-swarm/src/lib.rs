...
pub fn run_cli(cli: Cli) -> DsfbSwarmResult<PathBuf> {
    match cli.command {
        Command::Run(args) => {
            ...
            let config = RunConfig::resolve_with_patch(config_path.as_deref(), RunArgsPatch::from(args))
        ...
        Command::Scenario(args) => {
            ...
            let config = RunConfig::resolve_with_patch(
                config_path.as_deref(),
                RunArgsPatch::from(args),
            )
        ...
        Command::Benchmark(args) => {
            ...
            let config = BenchmarkConfig::resolve_with_patch(
                config_path.as_deref(),
                patch,
            )
