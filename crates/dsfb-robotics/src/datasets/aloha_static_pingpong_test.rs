//! aloha_static_pingpong_test adapter — see crate's docs for provenance.
//!
//! Real data flows through `scripts/preprocess_datasets.py` to
//! `data/processed/aloha_static_pingpong_test.csv`; the canonical entry point is
//! `paper-lock aloha_static_pingpong_test` which loads that CSV via
