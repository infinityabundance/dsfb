//! aloha_static_tape adapter — see crate's docs for provenance.
//!
//! Real data flows through `scripts/preprocess_datasets.py` to
//! `data/processed/aloha_static_tape.csv`; the canonical entry point is
//! `paper-lock aloha_static_tape` which loads that CSV via
